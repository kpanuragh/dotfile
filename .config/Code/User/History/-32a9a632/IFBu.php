...
class Import {
	...
	public static function events($import_file) {
		...
		$fields = [
			...
			"H" => "time_finish",
			"I" => "cost",
			"J" => "visible_learner_task",
			"K" => "zoom_meeting_address",
			"L" => "teams_meeting_address",
			"M" => "trainers"
		];
		...
		foreach ($data as $key => $entry) {
			...
			$entry["start_time"] = $entry["date"].$entry["time_start"];
			$entry["end_time"] = $entry["date"].$entry["time_finish"];
			$entry["end_time"] = $entry["date"].$entry["time_finish"];
			...
			//Create new event
		    $event=\Models\Schedule::createEvent($entry);
			if($entry["trainers"]){
			$entry["trainers"]=explode(",",$entry["trainers"]);

			//Add trainers Here

			foreach ($entry["trainers"] as $key => $manager) {
				$user = \Models\User
		   ...
		   if (isset($entry["venue_name"])) {
			$venue = \Models\Venue::where(
					'name',$entry["venue_name"])->first();

				\Models\TableExtension::updateField('schedules', $event, "venue_id",$venue);
		   }

			if(isset($entry['zoom_meeting_address']) && !empty($entry['zoom_meeting_address'])){
				$learningType=\Models\LearningModuleType
								::select('id')
								->where('slug', 'zoom_meeting')
								->first();
              $learningModule=new \Models\LearningModule;
			  $learningModule->name=$entry["name"].", Zoom meeting";
			  $learningModule->category_id=$category->id;
			  $learningModule->type_id=$learningType->id;
			  $learningModule->save();
			  $link = \Models\ScheduleLink::firstOrNew(
				[
					"schedule_id" => $event,
			...
			if(isset($entry['teams_meeting_address']) && !empty($entry['teams_meeting_address'])){
				$learningType=\Models\LearningModuleType
								::select('id')
								->where('slug', 'microsoft_teams')
								->first();
              $learningModule=new \Models\LearningModule;
			  $learningModule->name=$entry["name"].", Microsoft Teams";
			  $learningModule->category_id=$category->id;
			  $learningModule->type_id=$learningType->id;
			  $learningModule->save();
			  $link = \Models\ScheduleLink::firstOrNew(
			...
			$n_records['n_record_inserted']++;
					...
					$log[] = [
						'record' => [
							...
							'cost' => $entry['cost'],
							'visible_learner_task' => $entry['visible_learner_task'],
							'is_zoom' => $entry['is_zoom'],
							'is_teams' => $entry['is_teams'],
							'zoom_meeting_address' => $entry['zoom_meeting_address'],
							'teams_meeting_address' => $entry['teams_meeting_address'],
